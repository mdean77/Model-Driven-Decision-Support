...
public class Tutorial {
	...
	public static void main(String[] args) {
		...
		// 2) or populated manually:
		 ...
		 "jdbc:mysql://127.0.0.1:3306/glucoseTests");
...
//		hibernateProperties.setProperty(Environment.DIALECT, org.hibernate.dialect.HSQLDialect.class.getName());
		...
		dataStore.setEPackages(new EPackage[] { ApplicationPackage.eINSTANCE, GlucosePackage.eINSTANCE });
...

		{
			...
			System.out.println("List of glucose decisions:");
			System.out.println();
			query = session.createQuery("From ClinicalDecision");
			List<?> decisions = query.list();
			for (Iterator<?> it = decisions.iterator(); it.hasNext();) {
				ClinicalDecision decision = (ClinicalDecision) it.next();
				if (decision instanceof GlucoseDecision)
				System.out.println(decision.toString());
			}

			System.out.println();
			System.out.println("List of all decisions:");
			System.out.println();
			query = session.createQuery("From ClinicalDecision");
...
//			}
	...
	private static void addFakeData() {
		...
		GlucoseDecision glucose1 = GlucoseFactory.eINSTANCE.createGlucoseDecision();
		patient1.getDecisions().add(glucose1);
		GlucoseDecision glucose2 = GlucoseFactory.eINSTANCE.createGlucoseDecision();
		patient1.getDecisions().add(glucose2);
		...
		patient2.getDecisions().add(decision1);
